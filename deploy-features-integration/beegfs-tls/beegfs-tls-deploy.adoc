...
If all BeeGFS management services and clients have existing certificates signed by a trusted Certificate Authority (CA), you do not need to create or distribute new certificates.

=== Deploying a BeeGFS 8 cluster

For a new BeeGFS 8 cluster deployment, set the paths of the certificate and key files in the Ansible inventory's `user_defined_params.yml` file to point to the existing certificates:
...
=== Existing BeeGFS cluster

For an existing BeeGFS cluster, set the paths in the BeeGFS management services' configuration file to use the existing certificates:
...
=== Deploying a BeeGFS cluster

For new a BeeGFS 8 deployment, the `beegfs_8` Ansible role will handle creating a CA on the control node and generating the necessary certificates for the management services. This can be enabled by setting the following parameters in the Ansible inventory's `user_defined_params.yml` file:
...
For an existing BeeGFS cluster, edit the management service configuration file. For example, edit the file at `/mnt/mgmt_tgt_mgmt01/mgmt_config/beegfs-mgmtd.toml` and set:
