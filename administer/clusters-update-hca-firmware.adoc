...
TIP: For Lenovo ConnectX-7 adapters, use the `mlxfwmanager_LES` tool, which is available on NVIDIA’s link:https://network.nvidia.com/support/firmware/lenovo-intelligent-cluster/[OEM firmware^] page.
...
+
NOTE: Some firmware updates may require a reboot to apply the update. Refer to link:https://networking-docs.nvidia.com/mftswum/4201/mlxfwreset-loading-firmware-on-5th-generation-devices-tool#id-(4.20.1)mlxfwreset–LoadingFirmwareon5thGenerationDevicesTool-mlxfwresetLimitations[NVIDIA's mlxfwreset limitations^] for guidance. If a reboot is required, perform a reboot instead of resetting the adapters.
...
.. Restart the `eseries_nvme_ib.service`.
+
[source,console]
----
...
.. Verify the E-Series storage array's volumes are present.
[source,console]
----
multipath -ll
----
