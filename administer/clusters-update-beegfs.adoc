...
. Ensure your cluster is in a health state, with all BeeGFS services running on their preferred nodes. From a file node running BeeGFS services, verify all Pacemaker resources are running on their preferred nodes:
+
[source,console]
----
pcs status
...
The Pacemaker `beegfs-monitor` OCF scripts requires updates to support the new TOML configuration format introduced in BeeGFS 7.4.x and later versions.
