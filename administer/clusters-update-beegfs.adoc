...
| 7.4.6            | 8.2.2           | Yes       | Upgrade using the instructions in the <<upgrading-to-beegfs-82,Upgrading to BeeGFS 8.2>> section below.
|===

== Upgrading to BeeGFS 8.2

Before upgrading to BeeGFS 8, ensure your system is running at least BeeGFS 7.4.6. Any cluster running a release previous to BeeGFS 7.4.6 must follow the <<upgrading-to-beegfs-746-or-below,BeeGFS upgrade steps>> to upgrade to version 7.4.6.

BeeGFS 8 introduces several changes which require additional setup before upgrading from BeeGFS 7. This section guides you through preparing your cluster for BeeGFS 8's new features and requirements, including:
...
On all file nodes in the cluster, add the BeeGFS 8 package repository for your Linux distribution. Instructions for using the official BeeGFS repositories can be found at the link:https://www.beegfs.io/c/download/[BeeGFS download page]. Otherwise, configure your local beegfs mirror repository accordingly.
