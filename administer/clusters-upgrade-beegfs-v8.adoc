...
IMPORTANT: Before upgrading to BeeGFS v8, ensure your system is running at least BeeGFS 7.4.6. Any cluster running a release previous to BeeGFS 7.4.6 must first link:clusters-upgrade-beegfs.html[upgrade to version 7.4.6] before proceeding with this BeeGFS v8 upgrade procedure.
...
. Add the BeeGFS v8 package repository for your Linux distribution. Instructions for using the official BeeGFS repositories can be found at the link:https://www.beegfs.io/c/download/[BeeGFS download page]. Otherwise, configure your local BeeGFS mirror repository accordingly.
+
...
. Configure TLS for the client. TLS is required to use the BeeGFS CLI. Reference the link:../deploy-features-integration/beegfs-tls/beegfs-tls-deploy.html[Configure TLS Encryption for BeeGFS 8] procedure to configure TLS on the client.
