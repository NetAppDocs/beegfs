...
summary: "Update from BeeGFS 7.4.6 to BeeGFS 8."
---
= Upgrade to BeeGFS 8
:hardbreaks:
:nofooter:
...
[.lead]
Follow these steps to upgrade your BeeGFS HA cluster from version 7.4.6 to BeeGFS 8.
...
IMPORTANT: Before upgrading to BeeGFS 8, ensure your system is running at least BeeGFS 7.4.6. Any cluster running a release previous to BeeGFS 7.4.6 must first link:clusters-upgrade-beegfs.html[upgrade to version 7.4.6] before proceeding with this BeeGFS 8 upgrade procedure.
...
.. Refer to the link:https://doc.beegfs.io/7.4.6/advanced_topics/backup.html[BeeGFS Backup documentation^] for instructions on backing up your cluster configuration. At a minimum perform a backup of the management data directory:
+
[source,console]
----
