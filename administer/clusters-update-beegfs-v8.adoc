...
summary: "Upgrade from BeeGFS 7.4.6 to BeeGFS 8."
---
...
BeeGFS v8 introduces several significant changes which require additional setup before upgrading from BeeGFS v7. This procedure guides you through preparing your cluster for BeeGFS v8's new features and requirements.

IMPORTANT: Before upgrading to BeeGFS v8, ensure your system is running at least BeeGFS 7.4.6. Any cluster running a release previous to BeeGFS 7.4.6 must first link:clusters-update-beegfs-v7.html[upgrade to version 7.4.6] before proceeding with this BeeGFS v8 upgrade procedure.

== Tested upgrade path

The following upgrade path has been tested and verified:

[cols="1,1,1,5" options="header"]
|===
| Original Version | Upgrade Version | Multirail | Details
| 7.4.6            | 8.2             | Yes       | Upgrade from BeeGFS 7.4.6 to BeeGFS 8.2 using this procedure
|===
...
[IMPORTANT]
====
Upgrading to BeeGFS v8 requires cluster downtime. Plan your upgrade during a scheduled maintenance window to minimize impact on users and workloads.
====
...
[source,console]
----
cp -r /mnt/mgmt_tgt_mgmt01/data /mnt/mgmt_tgt_mgmt01/data_backup_$(date +%Y%m%d)
----
