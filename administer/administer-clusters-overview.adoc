...
While some of these concepts are introduced on the main link:beegfs-terms.html[terms and concepts] page, it is helpful to reintroduce them in the context of a BeeGFS HA cluster:
...
**Maintenance Mode:** Disables all resource monitoring and prevents Pacemaker from moving or otherwise managing resources in the cluster (also see the section on link:administer-clusters-maintenance-mode.html[maintenance mode^]).
