...
For additional details, see link:second-gen/beegfs-deploy-learn-ansible.html[Learn about the Ansible inventory].
...
For detailed procedures, see link:second-gen/beegfs-deploy-hardware.html[Deploy hardware].

.Software deployment
. link:second-gen/beegfs-deploy-setup-nodes.html[Set up file and block nodes].
...
. link:second-gen/beegfs-deploy-setting-up-an-ansible-control-node.html[Set up an Ansible control node].
. link:second-gen/beegfs-deploy-file-node-tuning.html[Tune system settings for performance].
. link:second-gen/beegfs-deploy-create-inventory.html[Create the Ansible inventory].
. link:second-gen/beegfs-deploy-define-inventory.html[Define Ansible inventory for BeeGFS building blocks].
. link:second-gen/beegfs-deploy-playbook.html[Deploy BeeGFS using Ansible].
. link:second-gen/beegfs-deploy-configure-clients.html[Configure BeeGFS clients].
...
* link:second-gen/beegfs-design-high-capacity-building-block.html[High Capacity Building Block]
