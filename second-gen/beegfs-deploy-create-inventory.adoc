...
. Create a subdirectory for cluster passwords and secure the file by encrypting it with Ansible Vault (see https://docs.ansible.com/ansible/latest/user_guide/vault.html[Encrypting content with Ansible Vault^]):
+
.. Create the subdirectory `group_vars/all`.
.. In the `group_vars/all` directory, create a passwords file labeled `passwords.yml`.
.. Populate the `passwords.yml file` with the following, replacing all username and password parameters according to your configuration:
+
....
...
....
+
.. Run `ansible-vault encrypt passwords.yml` and set a vault password when prompted.
...
....
+
NOTE: If the `ansible_ssh_user` is already `root`, then you can optionally omit the `ansible_become_password` and specify the `--ask-become-pass` option when running the playbook.
