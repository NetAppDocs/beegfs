...
a|Host card adapters may vary based on the file node's server model. Recommendations for verified file nodes include:
...
|RHEL 9.4 Server Physical with High Availability (2 socket).
...
|Drive Firmware
|Latest available for the drive models in use. See the link:https://mysupport.netapp.com/site/downloads/firmware/e-series-disk-firmware[E-Series disk firmware site^].
|===
