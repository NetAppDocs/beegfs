...
When creating the Ansible inventory file, follow these standard conventions (for more information, see link:beegfs-deploy-beegfs-general-config.html[Create the Ansible inventory]).
...
+
For example, in the first building block, file nodes h01 and h02 are both directly connected to block nodes a01 and c02. From top to bottom, the host names are h01, h02, a01, and a02.
...
+
The intent is to minimize the length of cable running to the top of rack switches, and to define a standard deployment practice to simplify troubleshooting. For data centers, this is not allowed due to concerns around rack stability. The inverse is certainly allowed, although cable lengths need to be adjusted accordingly.
