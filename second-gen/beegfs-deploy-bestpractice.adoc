...
When physically assembling and creating the Ansible inventory file, follow these standard conventions (for more information, see link:second-gen/beegfs-deploy-create-inventory.html[Create the Ansible inventory]).
...
This deployment guide will utilize a single subnet schema given the advantages listed in the link:second-gen/beegfs-design-software-architecture.html#beegfs-network-configuration[software architecture].
