...
+
The system settings vary depending on the server model you use as your file node. The settings must be manually configured based on the server model in use. To learn how to configure the system settings for the validated Lenovo SR665 V3 file nodes, see link:beegfs-deploy-file-node-tuning.html[Tune file node system settings for performance].
