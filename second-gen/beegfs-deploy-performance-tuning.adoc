...
+
You can override the default Linux OS system settings when you <<Step 4: Define the configuration for the common file node.>> The default settings were used to validate the BeeGFS on NetApp solution, but you can change them to adjust for your specific workloads or use cases.
...
== Performance tuning for the verfied file node --- Optional TASK for Deploy
