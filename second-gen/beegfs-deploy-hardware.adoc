...
+
Rack the building blocks so the two BeeGFS file nodes are above the BeeGFS block nodes. The following figure shows the correct hardware configuration for the BeeGFS building block using Lenovo ThinkSystem SR665 servers as the file nodes (rear view).
+
...
+
. Connect the file and block nodes using the link:beegfs-technology-requirements.html#file-node-cables[file node InfiniBand cables] to match the topology shown in the figure below.
+
[NOTE]
File nodes using Lenovo SR665 V3 servers will follow the same topology, but each HCA shown in the graphic will be shifted down one PCIe slot number.
+
...
+
. Connect the remaining InfiniBand ports on the file node to the storage network's InfiniBand switch using the link:beegfs-technology-requirements.html#switch-cables[switch InfiniBand cables].
+
