...
+
NOTE: The power supply configuration for production use cases should typically use redundant PSUs.
+
...
+
. Connect the block and file nodes using the link:beegfs-technology-requirements.html#file-node-requirements[1m InfiniBand HDR 200Gb direct attach copper cables], so that they match the topology shown in the following figure.
+
image:directattachcable-sr665v3.png["SR665V3 direct attach cabling"]
+
NOTE: The nodes across multiple building blocks are never directly connected. Each building block should be treated as a standalone unit and all communication between building blocks occurs through network switches.
+
. Connect the remaining InfiniBand ports on the file node to the storage network's InfiniBand switch using the link:beegfs-technology-requirements.html#storage-network-switch-requirements[2m InfiniBand cables] specific to your InfiniBand storage switch.
+
...
+
NOTE: The total number of building blocks that can be deployed in a single rack depends on the available power and cooling at each site.
