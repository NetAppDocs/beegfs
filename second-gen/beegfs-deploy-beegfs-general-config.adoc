...
+
You might have noticed that initially the file node  interface names that do match up with what is listed here (such as ib0 or ibs1f0). These custom names are configured in <<Step 4: Define the configuration for the common file node>>.
+
....
...
You can define a configuration common to a group of hosts under `group_vars` in a file name that corresponds with the group. Among other benefits, this prevents repeating a shared configuration in multiple places.
