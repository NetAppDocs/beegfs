...
. Using curl or your desired tool, download the packages for the version of OpenSM listed in the link:beegfs-technology-requirements.html[technology requirements^] section from NVIDIA’s website to the `<INVENTORY>/packages/` directory. For example:
+
[source,bash]
----
