...
For more details on standard conventions, best practices, and filled out example worksheets refer to the link:beegfs-deploy-bestpractice.html[best practices^] and link:beegfs-deploy-define-inventory.html[define BeeGFS building blocks^] sections of the BeeGFS on NetApp Verified Architecture.
